...
func TestMapIterSeek(t *testing.T) {
	...
	_ = all
	{
		...
		assert(t, vals[0] == 502 && vals[1] == 504)
	}
	{
		iter := tr.Iter()
		var vals []int
		...
		assert(t, vals[0] == 502 && vals[1] == 500)
	}
}
...
func TestMapDeepCopy(t *testing.T) {
	...
	// pairs of string.
	cols1 := NewMap[string, *Map[string, string]]()
	for i := 0; i < Ncols; i++ {
		col := NewMap[string, string]()
		for j := 0; j < Nvals; j++ {
	...
	assert(t, count1 == Ncols*Nvals)
	assert(t, count2 == Ncols*Nvals/2)
